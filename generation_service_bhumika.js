...
export const generate30DayStrategy = async (workspaceId, { maxDays = null } = {}) => {
  try {
    ...
    // Calculate actual days G�� cap by maxDays for free plan, otherwise use user's selected duration
...
export const generateContentForSpecificRow = async (workspaceId, entryId) => {
  ...
  // =���n+� Ensure complete isolation between Content and Hashtag regeneration.
...
const applyVisualOverlays = async (imageUrl, logoUrl, headingText, subheadingText, aspectRatio = '1:1') => {
  if (!logoUrl && !headingText && !subheadingText) {
    console.log('    [VisualOverlay] Gšn+�  Skipping G�� no text or logo to overlay.');
    return imageUrl;
  }

  console.log('    [VisualOverlay] =��+n+�  Applying overlays (Logo + Text)...');
  const overlayStart = Date.now();

  try {
    // 1. Download the generated image as base64
    //    Use a longer timeout (45 s) G�� full-res Gemini images can be several MB.
    ...
    if (!imageData || imageData.byteLength < 100) {
      console.warn(`    [VisualOverlay] G��n+�  Downloaded image is empty/too small (${imageData?.byteLength ?? 0} bytes) G�� skipping overlay.`);
      return imageUrl;
    }

    const imageBase64 = Buffer.from(imageData).toString('base64');
    // Force a valid image MIME G�� GCS signed URLs often return application/octet-stream
      ...
      try {
        ...
        if (rawContentType.includes('icon') || logoUrl.toLowerCase().endsWith('.ico')) {
          try {
            ...
            console.log(`    [VisualOverlay] G�� Successfully converted ICO to ${rawContentType}`);
          } catch (e) {
            console.warn(`    [VisualOverlay] G��n+� Failed to fetch PNG equivalent for ICO: ${e.message}`);
          }
        }

        if (rawBuffer.byteLength >= 100) {
          // Strategy 1: sharp direct conversion G�� always outputs PNG (Gemini-supported)
          try {
            ...
            // Strategy 2: sharp with failOn:none G�� handles partially corrupt files
            try {
              ...
              if (GEMINI_SUPPORTED_MIMES.includes(rawContentType)) {
                ...
                // ICO, BMP, SVG, TIFF etc G�� Gemini rejects these, skip logo entirely
                console.warn(`    [VisualOverlay] G��n+�  Logo format "${rawContentType}" is not supported by Gemini G�� skipping logo, text overlay will still apply.`);
              }
            }
          }
        } else {
          console.warn('    [VisualOverlay] G��n+�  Logo downloaded but appears empty G�� skipping logo.');
        }
      } catch (e) {
        console.warn(`    [VisualOverlay] G��n+�  Logo download failed (${e.message}), continuing with text only.`);
      }
    }
    ...
    //    IMPORTANT: use 'global' location G�� same as generateImageFromPrompt uses;
...
* Output the final composited image ONLY. No conversational text.`;
    ...
    while (true) {
      try {
        ...
        if (retryCount < maxRetries && isQuotaError) {
          retryCount++;
          console.warn(`    [VisualOverlay] G��n+�  Quota hit (429). Retrying ${retryCount}/${maxRetries} after ${retryCount * 4}s...`);
    ...
    if (!resultBase64) {
      console.warn('    [VisualOverlay] G��n+�  Gemini returned no image G�� falling back to original.');
      return imageUrl;
    }
    ...
    if (!gcsResult?.publicUrl) {
      console.warn('    [VisualOverlay] G��n+�  GCS upload failed G�� falling back to original.');
      return imageUrl;
    }

    console.log(`    [VisualOverlay] G�� Overlays composited in ${Date.now() - overlayStart}ms G�� ${gcsResult.publicUrl.substring(0, 60)}...`);
    return gcsResult.publicUrl;

  } catch (err) {
    console.error(`    [VisualOverlay] G�� Overlay failed (${err.message}) G�� using original image.`);
    return imageUrl;
  }
};

/**
 * AI ADS AGENT G�� VISUAL POST GENERATION PIPELINE
 * G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
 * Step 1   G�� GPT-4    G�� Brand-aware Imagen prompt engineering
 * Step 2   G�� Vertex AI Imagen 3/4 G�� High-quality visual render
 * Step 2.5 G�� Gemini 2.5 Flash G�� Brand logo overlay (top-left)
 * Step 3   G�� GCS      G�� Secure cloud storage
 * Step 4   G�� MongoDB  G�� GeneratedAsset + Job update
 * Step 5   G�� Calendar G�� Entry status marked "generated"
 */
export const generateVisualPostForEntry = async (workspaceId, entryId, jobId, modelId = 'imagen-3.0-generate-001', postFormat = 'single', aspectRatio = '1:1', carouselCount = 3) => {
  ...
  console.log('\n' + 'G��'.repeat(60));
  console.log('=�Ŀ  AI ADS AGENT G�� VISUAL POST PIPELINE STARTED');
  console.log('G��'.repeat(60));
  console.log(`  =��� Entry ID    : ${entryId}`);
  console.log(`  =��� Workspace   : ${workspaceId}`);
  console.log(`  =��� Job ID      : ${jobId}`);
  console.log(`  =��� Model       : ${modelId}`);
  console.log(`  =��+n+�  Format      : ${postFormat.toUpperCase()}`);
  console.log(`  GŦ  Started at  : ${new Date().toISOString()}`);
  console.log('G��'.repeat(60));

  // G��G�� LOAD: Brand Profile & Calendar Entry G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  if (!brand || !entry) {
    console.error(`[VisualPost] G�� ABORT G�� Data missing: brand=${!!brand}, entry=${!!entry}`);
  ...
  const normalizeAssetType = (raw) => {
    ...
    // Everything else (image, informative, promotional, educational, awareness, etc.) G�� image
    return 'image';
  };
  const postType = normalizeAssetType(rawPostType);

  console.log(`    G�� Brand        : "${companyName}"`);
  console.log(`    G�� Post title   : "${title}"`);
  console.log(`    G�� Platform     : ${platform} | Type: ${postType} | Phase: ${phase}`);
  console.log(`    G�� Brand colors : ${brandColors}`);
  console.log(`    G�� Tone         : ${tone}`);
  console.log(`    GŦ  Loaded in ${Date.now() - dataLoadStart}ms`);

  // G��G�� STEP 1: GPT-4 Prompt Engineering G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  if (isCarousel) {
    ...
    if (carouselCount === 2) {
      slideStructure += '- Slide 2: CTA-driven closing G�� inspiring action with brand energy';
    } else if (carouselCount === 3) {
      slideStructure += '- Slide 2: Solution / product in context G�� aspirational lifestyle\n';
      slideStructure += '- Slide 3: CTA-driven closing G�� inspiring action with brand energy';
    } else if (carouselCount === 4) {
      slideStructure += '- Slide 2: Problem visualization G�� what challenge the audience faces\n';
      slideStructure += '- Slide 3: Solution / product in context G�� aspirational lifestyle\n';
      slideStructure += '- Slide 4: CTA-driven closing G�� inspiring action with brand energy';
    } else {
      slideStructure += '- Slide 2: Problem visualization G�� what challenge the audience faces\n';
      slideStructure += '- Slide 3: Solution / product in context G�� aspirational lifestyle\n';
      slideStructure += '- Slide 4: Key benefit or proof point G�� data, result, transformation\n';
      slideStructure += '- Slide 5: CTA-driven closing G�� inspiring action with brand energy';
    }
  }
...
Generate ${carouselCount} COMPLETELY DIFFERENT Imagen 3 image generation prompts for a CAROUSEL post.
CRITICAL: Each slide MUST show a DIFFERENT scene, subject, composition, and setting. They should NOT look similar at all.
...
- NO text, words, letters, numbers, or logos in any image
- EACH slide must have a UNIQUE subject, camera angle, background, and mood
- Vary the setting: e.g. one outdoor, one indoor, one close-up, one wide shot
- Each prompt should be at least 3-4 detailed sentences long

STRICT FORMAT — output EXACTLY like this (number followed by period and space):
1. [detailed prompt for slide 1 here]
2. [detailed prompt for slide 2 here]
3. [detailed prompt for slide 3 here]
...

Output ONLY ${carouselCount} numbered prompts. No JSON, no explanation, no extra text.`
    : `You are an expert AI Image Prompt Engineer for social media advertising.
...
Output ONLY the raw Imagen prompt text, nothing else. No JSON, no explanation.`;
  ...
  if (!imagenPrompt || imagenPrompt.trim().length < 20) {
    console.error('[VisualPost] G�� GPT-4 returned empty/invalid prompt');
    throw new Error('GPT-4 returned an empty image prompt');
  }

  const trimmedPrompt = imagenPrompt.trim();
  console.log(`    G�� Prompt received (${trimmedPrompt.length} chars) in ${Date.now() - promptStart}ms`);
  
  let finalImagePrompt = trimmedPrompt;
  let carouselSlides = [];

  if (isCarousel) {
    console.log(`    =\u2699\ufe0f Carousel mode \ud83d\udc49 Parsing ${carouselCount} slide prompts...`);
    console.log(`    =\ud83d\udcdd Raw LLM output (first 500 chars): "${trimmedPrompt.substring(0, 500)}"`);

    // Strategy 1: Match numbered patterns like "1. ", "2. ", "1) ", "Slide 1:", etc.
    const numberedRegex = /(?:^|\n)\s*(?:Slide\s*)?\d+[.):]\s*/gi;
    let slideMatches = trimmedPrompt.split(numberedRegex).filter(s => s.trim().length > 15);
    console.log(`    =\ud83d\udcca Strategy 1 (numbered split): found ${slideMatches.length} chunks`);

    // Strategy 2: If numbered split didn't get enough, try double-newline split
    if (slideMatches.length < carouselCount) {
      const doubleNewlineSplit = trimmedPrompt.split(/\n\s*\n/).filter(s => s.trim().length > 15);
      console.log(`    =\ud83d\udcca Strategy 2 (double-newline): found ${doubleNewlineSplit.length} chunks`);
      if (doubleNewlineSplit.length >= carouselCount) {
        slideMatches = doubleNewlineSplit;
      }
    }

    // Strategy 3: If still not enough, try single-newline split on long lines
    if (slideMatches.length < carouselCount) {
      const lineSplit = trimmedPrompt.split('\n').filter(l => l.trim().length > 40);
      console.log(`    =\ud83d\udcca Strategy 3 (line-split): found ${lineSplit.length} long lines`);
      if (lineSplit.length >= carouselCount) {
        slideMatches = lineSplit;
      }
    }

    // Clean up: strip leading "1. ", "Slide 1:", etc. from each match
    slideMatches = slideMatches.map(s => s.replace(/^\s*(?:Slide\s*)?\d+[.):]?\s*/i, '').trim());

    // Take exactly carouselCount
    carouselSlides = slideMatches.slice(0, carouselCount);

    // Final fallback: if we still don't have enough, duplicate the base prompt with variation hints
    if (carouselSlides.length < carouselCount) {
      console.warn(`    \u26a0\ufe0f  Only got ${carouselSlides.length}/${carouselCount} slide prompts. Adding variation suffixes.`);
      const variationSuffixes = [
        ', shot from a wide angle with warm golden hour lighting',
        ', extreme close-up detail shot with soft bokeh background',
        ', overhead flat-lay composition with natural shadows',
        ', dynamic action shot with motion blur and vibrant energy'
      ];
      const basePrompt = carouselSlides[0] || trimmedPrompt;
      while (carouselSlides.length < carouselCount) {
        const suffixIdx = carouselSlides.length - 1;
        const suffix = variationSuffixes[suffixIdx % variationSuffixes.length];
        carouselSlides.push(basePrompt + suffix);
      }
    }

    // Log each parsed prompt for debugging
    carouselSlides.forEach((p, i) => console.log(`       Slide ${i+1} prompt: "${p.substring(0, 80)}..."`));

    // Use the first slide as the representative "cover" prompt for legacy single-image fields
    finalImagePrompt = carouselSlides[0] || trimmedPrompt;
    console.log(`    \u2705 Successfully parsed ${carouselSlides.length} distinct slide prompts.`);
  }

  // G��G�� STEP 2: Vertex AI Imagen Generation G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  const SLIDE_FRAMES = [
    ...
    { role: 'CTA',      prefix: '',                   suffix: ' G�� Act Now' },
  ];
  const buildLocalVariations = (count) => {
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        ...
        caption: `This is slide ${i + 1} of ${count}. ${title}`,
  ...
  if (isCarousel && carouselSlides.length > 0) {
    console.log(`    G�� Staggered Rendering ${carouselSlides.length} slides (1.2s apart to manage quota)...`);
...
Rules:
- Each "heading" must be DIFFERENT from the others G�� no repetition
- Keep headings under 8 words G�� punchy, bold, suitable for image overlay
- Keep subheadings under 15 words

Rules (additional):
  - "caption": Write a unique, engaging Instagram-style caption for this specific slide (1-2 sentences).
  - "hook": Write a catchy hook line for this specific slide (under 10 words).
  - "cta": Write a short Call-To-Action for this slide (e.g. 'Swipe Left', 'Save This', 'Read More').
...
[
  ...
  { "heading": "...", "subheading": "...", "caption": "...", "hook": "...", "cta": "..." }
    ...
    try {
      ...
      console.log(`    G�� Slide text variations ready (${slideTexts.length} slides):`);
      slideTexts.forEach((s, i) => console.log(`       Slide ${i+1}: "${s.heading}" / "${s.subheading}"`) );
    } catch(e) {
      console.warn(`    G��n+� Vertex variation call failed (${e.message}) G�� using smart local fallback.`);
    ...
    for (let i = 0; i < carouselSlides.length; i++) {
      ...
      try {
        ...
        if (rawSlideUrl) {
          // G��G�� STEP 2.5: Apply brand logo and text overlay to each slide G��G��
          ...
          console.warn(`       G��n+�  Slide ${i+1} returned empty URL`);
        }
      } catch (err) {
        console.error(`       G�� Slide ${i+1} failed: ${err.message}`);
      }
    ...
    console.log(`    G�� ${generatedSlides.length}/${carouselSlides.length} slides rendered successfully.`);
  } else {
    // Single image generation
    console.log(`    =��� Aspect ratio : ${aspectRatio}`);
    const rawImageUrl = await generateImageFromPrompt(finalImagePrompt, null, aspectRatio, selectedModel);
    // G��G�� STEP 2.5: Apply visual overlays (logo + text) G��G��
    imageUrl = await applyVisualOverlays(rawImageUrl, brand.logoUrl, title, hook, aspectRatio);
  }

  if (!imageUrl && generatedSlides.length === 0) {
    console.error('[VisualPost] G�� Vertex AI returned no image URL');
    throw new Error('Vertex AI Imagen returned no image URL');
  }

  const imagenMs = Date.now() - imagenStart;
  console.log(`    G�� Generation cycle complete in ${imagenMs}ms`);
  console.log(`    =��+n+�  Logo overlay : ${brand.logoUrl ? 'Applied' : 'Skipped (no logo)'}`);
  console.log(`    =��� Final images  : ${isCarousel ? generatedSlides.length + ' slides' : '1 single image'}`);

  // G��G�� STEP 3: Save GeneratedAsset to DB G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  console.log(`    G�� GeneratedAsset saved: ${asset._id}`);
  console.log(`    =��� Asset name   : ${assetName} | Slides: ${generatedSlides.length}`);
  console.log(`    GŦ  Saved in ${Date.now() - assetStart}ms`);

  // G��G�� STEP 4: Mark GenerationJob as Completed G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  console.log(`    G�� Job ${jobId} G�� status: "completed"`);

  // G��G�� STEP 5: Mark CalendarEntry G�� LEAVE STATUS UNCHANGED FOR ISOLATION G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  ...
  // await entry.save();
  console.log(`    G�� Entry ${entryId} status preserved for content generation isolation`);

  // G��G�� PIPELINE COMPLETE G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
  const totalMs = Date.now() - pipelineStart;
  const totalSec = (totalMs / 1000).toFixed(1);
  console.log('\n' + 'G��'.repeat(60));
  console.log('G��  AI ADS AGENT G�� PIPELINE COMPLETE');
  console.log('G��'.repeat(60));
  console.log(`  =��� Asset ID     : ${asset._id}`);
  console.log(`  =��� Model used   : ${selectedModel}`);
  console.log(`  GŦ  Total time   : ${totalSec}s (${totalMs}ms)`);
  console.log(`  =��� Image URL    : ${imageUrl.substring(0, 80)}...`);
  console.log('G��'.repeat(60) + '\n');

  logger.info(`[VisualPost] G�� Pipeline complete in ${totalSec}s | AssetID=${asset._id} | Model=${selectedModel}`);
  return asset;
};
