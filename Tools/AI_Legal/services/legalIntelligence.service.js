...
export const analyzeCaseDetails = async (rawText, currentData = {}, language = 'English') => {
  const langRule = language === 'Hindi' 
    ? '### LANGUAGE RULE:\n- Generate ALL JSON string values (summaries, event descriptions, evidence strengths, law descriptions, process steps, risk assessments, vulnerabilities, strategies, relief, and recommendations) strictly in professional, native legal Hindi (Devanagari script).\n- The JSON keys must remain in English as defined below.\n- Keep proper nouns, registration numbers, case numbers, statute numbers, and section numbers in their official format (e.g. IPC Section 420 as IPC धारा 420).'
    : '';
