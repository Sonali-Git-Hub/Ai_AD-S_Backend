import { r as reactExports, cA as useLocation, v as useLanguage, cB as chatStorageService, c8 as generateChatResponse, j as jsxRuntimeExports, N as ArrowLeft, bX as Download, cg as exportToPDF, z as zt, b$ as History, bR as Plus, t as Scale, e as ChevronRight, cq as RotateCcw, P as Check, cc as ShieldAlert, cs as Square, X, cC as Paperclip, T as Trash2, bZ as Copy, cm as Mic, o as Send, A as AnimatePresence, m as motion, ac as ChevronDown, ae as Search, c9 as Briefcase, ce as PinOff, cd as Pin, ci as SlidersHorizontal, M as MessageSquare, bS as useOutputLanguage, ai as Markdown, cD as remarkGfm, cE as ThumbsUp, cF as ThumbsDown, cG as Volume2, b_ as Share2, bJ as Landmark, F as FileText, cH as CircleHelp, E as ExternalLink, k as Sparkles, b as Clock, S as Settings, cI as Image, a as apiService, O as React } from "./index-DjJL8ne1.js";
...
"${precedingText}"`;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full flex flex-col", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "legal-msg-ai-text relative flex-1 text-slate-800 text-[14px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          Markdown,
          {
            ...
            children: formatMarkdownForParsing(getDisplayText(msg.text))
          }
        )
      }
    ) }),
...
Please continue the conversation naturally using this context. Never ask the user to re-specify which law or section they are referring to.`;
  ...
  const [chatVoiceState, setChatVoiceState] = reactExports.useState("idle");
  const [chatVoiceText, setChatVoiceText] = reactExports.useState("");
  const [chatVoiceBlob, setChatVoiceBlob] = reactExports.useState(null);
  const [chatVoiceDuration, setChatVoiceDuration] = reactExports.useState(0);
  const chatMediaRecorderRef = reactExports.useRef(null);
  const chatAudioContextRef = reactExports.useRef(null);
  const chatAnalyserRef = reactExports.useRef(null);
  const chatDataArrayRef = reactExports.useRef(null);
  const chatSourceRef = reactExports.useRef(null);
  const chatCanvasRef = reactExports.useRef(null);
  const chatAnimationIdRef = reactExports.useRef(null);
  const chatAudioChunksRef = reactExports.useRef([]);
  const chatDurationIntervalRef = reactExports.useRef(null);
  const chatRecognitionRef = reactExports.useRef(null);
  const drawChatWaveform = reactExports.useCallback(() => {
    ...
    ctx.fillStyle = isDark2 ? "rgba(15, 22, 42, 0.2)" : "rgba(248, 250, 252, 0.2)";
    ...
    if (chatAnalyserRef.current) {
      ...
      for (let i = 0; i < dataArray.length; i++) {
        ...
        ctx.fillStyle = isDark2 ? "rgba(99, 102, 241, 0.8)" : "rgba(79, 70, 229, 0.8)";
        ctx.fillRect(x, canvas.height / 2 - barHeight / 2, barWidth, barHeight);
        x += barWidth + 1;
      }
    } else {
      ctx.lineWidth = 1.5;
      ctx.strokeStyle = isDark2 ? "#6366F1" : "#4F46E5";
      ctx.beginPath();
        ...
        reader.onloadend = async () => {
          ...
          try {
            ...
            if (data && data.text) {
              ...
              zt.success("Speech transcribed successfully.");
            } else {
            ...
            if (chatVoiceText && chatVoiceText.trim().length > 0) {
              setChatVoiceState("preview");
              zt.error("AI transcription failed. Fell back to browser transcript.");
            } else {
  ...
  const chatStreamRef = reactExports.useRef(null);
  ...
  const handleTextSelection = reactExports.useCallback((e) => {
    if (e.target.closest(".smart-context-tooltip") || e.target.closest("button") || e.target.closest("a") || e.target.closest("input") || e.target.closest("textarea") || e.target.closest("header") || e.target.closest("footer")) {
      return;
    }
  ...
  const loadSessionHistory = async (sessionId) => {
    try {
      ...
      const messagesList = history && Array.isArray(history.messages) ? history.messages : Array.isArray(history) ? history : [];
  ...
  reactExports.useEffect(() => {
    const loadSessions = async () => {
      var _a, _b;
      console.log("[LegalChatScreen] loadSessions triggered. currentCase?._id:", currentCase == null ? void 0 : currentCase._id);
      try {
        const dbSessions = await chatStorageService.getSessions(null, "GENERAL");
        const filteredDb = dbSessions.filter(
          (s) => s.activeTool === "General Legal Chat" || !s.activeTool || s.activeTool === "General Chat" || s.activeTool === "NORMAL_CHAT"
        );
        ...
        setSessions(mapped);
        const caseId = (currentCase == null ? void 0 : currentCase._id) || (currentCase == null ? void 0 : currentCase.id) || "general";
        const storageKey = `aisa_active_legal_chat_session_id_${caseId}`;
        const activeId = localStorage.getItem(storageKey);
        const isPathNew = window.location.pathname.endsWith("/new") || ((_a = location.state) == null ? void 0 : _a.newChat);
        if (isPathNew) {
          ...
          await handleNewChat(false);
          if ((_b = location.state) == null ? void 0 : _b.newChat) {
            try {
...
`;
      ...
      const detectedLanguage = detectPreferredLanguage(promptText2, messages, toolkitLanguage);
      const isSwitch = isLanguageSwitchQuery(promptText2) ? "Yes" : "No";
      systemInstruction += `
...
`;
      const response = await generateChatResponse(
        ...
        "LEGAL_TOOLKIT",
        chatIdRef.current,
        (currentCase == null ? void 0 : currentCase._id) || null
      );
      let responseText = "";
...
`;
  ...
  const handleNewChat = async (param) => {
    ...
    if (currentCase && isAutoAnalysis) {
      ...
      setMessages([userMsg]);
      setIsTyping(true);
      const newSessionItem = { chat_id: newId, title: "Case Analysis", timestamp: Date.now(), preview: promptText2 };
...
`;
        const detectedLanguage = detectPreferredLanguage(promptText2, [], toolkitLanguage);
        const isSwitch = isLanguageSwitchQuery(promptText2) ? "Yes" : "No";
        systemInstruction += `
...
- Do not mix Hindi and English in the same sentence.
`;
        const response = await generateChatResponse([], promptText2, systemInstruction, [], detectedLanguage, null, "LEGAL_TOOLKIT", newId, (currentCase == null ? void 0 : currentCase._id) || null);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full w-full bg-[#FAFBFD] font-sans relative overflow-hidden select-text text-slate-800", children: [
          ...
          messages.filter((m) => !m.isIntro).length === 0 ? (
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 md:space-y-8 py-2 md:py-6 max-w-[760px] mx-auto w-full text-center font-sans select-none flex flex-col items-center mt-6 md:mt-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col items-center justify-center gap-3 md:gap-4 text-center my-4 md:my-6", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("h1", { className: "text-2xl md:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "⚖️" }),
                  " AI Legal Assistant"
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs sm:text-sm font-bold text-[#4F46E5] uppercase tracking-[0.2em] mt-1", children: "Start a new legal conversation" })
              ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "shrink-0 bg-gradient-to-t from-[#FAFBFD] via-[#FAFBFD]/95 to-transparent pt-3 md:pt-4 px-2 sm:px-4 border-t border-slate-200/60 sticky bottom-0 z-30", style: { paddingBottom: "calc(env(safe-area-inset-bottom, 16px) + 12px)" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[960px] mx-auto space-y-3 w-full", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white border border-[#E5E7EB] rounded-full p-1.5 md:p-2 flex items-center gap-1 sm:gap-2 shadow-md hover:shadow-lg transition-shadow relative w-full", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "form",
          {
            ...
            children: micPermissionError ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex items-center justify-between gap-3 p-1 select-none w-full text-left", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 min-w-0 flex-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(MicOff, { size: 16, className: "text-red-550 shrink-0" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-red-500 font-bold leading-tight truncate", children: "Microphone Access is required." })
              ] }),
            ...
            ] }) : chatVoiceState === "recording" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col gap-2 min-w-0 w-full text-left", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-3 w-full", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 shrink-0", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-2 h-2 rounded-full bg-rose-500 animate-ping shrink-0" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-black uppercase text-rose-500 tracking-wider whitespace-nowrap hidden sm:inline", children: "● Listening" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-black uppercase text-rose-500 tracking-wider whitespace-nowrap sm:hidden", children: "● REC" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 max-w-[240px] h-7 bg-transparent rounded-lg overflow-hidden border dark:border-zinc-800/80 p-0.5 shrink-0 min-w-[80px]", children: /* @__PURE__ */ jsxRuntimeExports.jsx("canvas", { ref: chatCanvasRef, className: "w-full h-full bg-transparent", width: 240, height: 24 }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-[13px] font-mono font-bold text-slate-700 dark:text-slate-305 shrink-0", children: [
              ...
              chatVoiceText && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full text-left px-3 py-1.5 bg-black/5 dark:bg-black/20 rounded-xl max-h-16 overflow-y-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs font-semibold text-slate-655 dark:text-slate-300 italic", children: [
                '"',
                chatVoiceText,
                '"'
              ] }) })
            ] }) : chatVoiceState === "transcribing" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex items-center justify-center gap-2 py-1 select-none", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin shrink-0" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-black uppercase text-indigo-500 tracking-widest animate-pulse whitespace-nowrap", children: "Transcribing Speech to Text..." })
            ] }) : chatVoiceState === "preview" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col gap-2 p-1 select-none w-full text-left", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[9px] font-black uppercase tracking-wider text-[#5B3DF5]", children: "You Said (Voice Transcript Preview):" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-end gap-2.5 w-full", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "textarea",
                  {
                    rows: 2,
                    ...
                    className: `flex-1 text-xs font-semibold p-2 border rounded-xl outline-none resize-none focus:border-indigo-500 ${isDark ? "bg-zinc-950 border-zinc-800 text-white" : "bg-slate-50 border-slate-205 text-slate-700"}`
                  }
                ),
                        ...
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "button",
                            {
                              ...
                              children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate text-xs text-slate-800 dark:text-white font-bold", children: s.title || "New Chat" }),
                                s.preview && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-slate-450 dark:text-slate-500 font-medium truncate max-w-[200px] block", children: s.preview }),
                                /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-[9px] text-slate-400 font-medium font-mono block", children: [
                                  "Last Updated: ",
